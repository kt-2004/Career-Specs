...
def register(request):
    ...
    if request.method=='POST':
        ...
        if request.POST.get('act', 'login') == 'register': #For Register...
            ...
            return response
        elif request.POST.get('act', 'login') == 'login': #For Login...
            print("logging in...")
            try:
...
def mcq(request):
    ...
    with connection.cursor() as cursor:
        for i in student_obj.get_interested_subjects_list():
            cursor.execute(f"SELECT * FROM csweb_MCQ WHERE subject='{i}'")
        ...
        for j in student_obj.get_skills_list():
            cursor.execute(f"SELECT * FROM csweb_MCQ WHERE subject='{j}'")
            t2 = list(cursor.fetchall())
            print(j,t2)
            temp+=random.sample(t2,3)
            t2=[]
        cursor.execute(f"SELECT * FROM csweb_MCQ WHERE subject='{student_obj.subject1}'")
        temp +=  random.sample(list(cursor.fetchall()),3)
        cursor.execute(f"SELECT * FROM csweb_MCQ WHERE subject='{student_obj.subject2}'")
        temp +=  random.sample(list(cursor.fetchall()),3)
        cursor.execute(f"SELECT * FROM csweb_MCQ WHERE subject='{student_obj.subject3}'")
        temp +=  random.sample(list(cursor.fetchall()),3)
    questions = []
    for i in temp:
...
"""
    ...
    with connection.cursor() as cursor:
        for stuff in dict.items():
            try:
                cursor.execute(f"SELECT * FROM csweb_stream where name='{stuff[0]}';")
                items = list(cursor.fetchall())
                if len(items)<=0:
                    raise IndexError
                cursor.execute(f'UPDATE csweb_stream SET description="{stuff[1]}" WHERE name="{stuff[0]}";')
